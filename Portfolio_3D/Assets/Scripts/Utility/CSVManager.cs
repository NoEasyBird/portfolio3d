...
namespace Utility
{
    public static class CSVManager
    {
        ...
        {
            TextAsset textAsset = path.LoadAsset<TextAsset>();
            if (textAsset)
            {
                return Read<T>(textAsset);
            }
            
            Debug.LogError("Read Text Asset Error - Path : " + path);
            ...
            for (var i = 1; i < parseData.GetLength(0); i++)
            {
                ...
                for (var j = 0; j < parseData.GetLength(1); j++)
                {
                    ...
                    if (propertyInfo != null)
                    {
                        ...
                        {
                            Debug.LogError(string.Format("[{0}][{1}][{2}]{3}", typeof(T), fieldName, i, e.Message));
                        }
                        ...
                        {
                            Debug.LogError(string.Format("[{0}][{1}][{2}] Exception", typeof(T), fieldName, i));
                        }
                    }
                }
                objList.Add(obj);
        ...
        private static bool PassNullData(string[,] parseData, int lineIndex)
        {
            bool isAllNull = true;
            for (var i = 0; i < parseData.GetLength(1); i++)
            {
                if (!string.IsNullOrEmpty(parseData[lineIndex, i]))
                {
                    isAllNull = false;
                    break;
                }
